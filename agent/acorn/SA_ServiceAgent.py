...
from zkp.utils.utils import ModP, mod_hash
...
from zkp.rangeproofs.rangeproof_aggreg_verifier import AggregRangeVerifier
...
class SA_ServiceAgent(Agent):
    ...
    def reconstruction(self, currentTime):
        ...
        for i in self.online_set:
            ...
            mi_recon, _ = points_to_secret_int(mi_shares, prime=self.secret_sharing_prime, isecc=0)
            # 使用动态字节长度
            share_bytes_length = (self.secret_sharing_prime.bit_length() + 7) // 8
        ...
        for i in self.offline_set:
            ...
            ai_recon, _ = points_to_secret_int(ai_shares, prime=self.secret_sharing_prime, isecc=0)
    ...
    def _verify_range_proofs(self):
        ...
        if use_traditional:
            ...
            for client_id, proof in self.range_proofs.items():
                if client_id in self.range_proof_commitments:
                    commitment = self.range_proof_commitments[client_id]
                    verifier = AggregRangeVerifier([commitment], self.g, self.h, self.gs, self.hs, self.u, proof)
                    try:
                        verifier.verify()
                    except Exception as e:
    ...
    def _verify_inner_product_proofs(self):
        ...
        for client_id, proof in self.inner_product_proofs.items():
            if client_id in self.inner_product_commitments:
                commitment = self.inner_product_commitments[client_id]
                try:
                    result = self.inner_product_verifier.verify(proof, commitment)
                    if not result:
                        raise ValueError(f"Client {client_id} inner product proof verification failed")
                except Exception as e:
                    raise ValueError(f"Client {client_id} inner product proof verification failed: {e}")
