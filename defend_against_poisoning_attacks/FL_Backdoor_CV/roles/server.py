...
from FL_Backdoor_CV.roles.aggregation_rules import jza
from FL_Backdoor_CV.roles.aggregation_rules import jzx
from FL_Backdoor_CV.roles.aggregation_rules import jzx_test
from FL_Backdoor_CV.roles.aggregation_rules import jzx_no_defense
from FL_Backdoor_CV.roles.aggregation_rules import zero_mask
...
class Server:
    ...
    def train_and_aggregate(self, global_lr):
        ...
        # === aggregate ===
        global_update = None
        if args.aggregation_rule.lower() == 'jzx':
            global_update, eta, l2_norm_global, linf_norm_abs_mean, b = jzx(model_updates)
            eta_path = os.path.join('./eta', f"eta_{self.path}")
            l2_path = os.path.join('./l2', f"l2_{self.path}")
            linf_path = os.path.join('./linf', f"linf_{self.path}")
            b_path = os.path.join('./b', f"b_{self.path}")

            with open(eta_path, 'a') as f:
                f.write(str(eta) + '\n')
            with open(l2_path, 'a') as f:
                f.write(str(l2_norm_global) + '\n')
            with open(linf_path, 'a') as f:
                f.write(str(linf_norm_abs_mean) + '\n')
            with open(b_path, 'a') as f:
                f.write(str(b) + '\n')

        elif args.aggregation_rule.lower() == 'jzx_test':
            global_update, self.l2_norm, self.linf_norm, self.linf_norm_shprg, self.b, benign_index \
                = jzx_test(model_updates, self.l2_norm, self.linf_norm, self.linf_norm_shprg, self.b, self.current_round)
            print(f"b: {self.b}, benign_index: {benign_index}\n")
            
            self.benign_index_list.append(benign_index)
            benign_index_path = os.path.join('./benign_index', f"{args.weight}_{self.path}")
            # if self.current_round == args.rounds:
            if args.resume == 1 and self.current_round == args.rounds + args.retrain_rounds:
                with open(benign_index_path, 'a') as f:
                    f.write(str(self.benign_index_list) + '\n')
                    f.write(str(sum(self.benign_index_list) / len(self.benign_index_list)) + '\n')
            
            b_path = os.path.join('./b', f"{args.weight}_{self.path}")
            with open(b_path, 'a') as f:
                f.write(str(self.b) + '\n')

        elif args.aggregation_rule.lower() == 'jzx_no_defense':
            global_update, self.l2_norm, self.linf_norm, self.linf_norm_shprg, self.b, benign_index \
                = jzx_no_defense(model_updates, self.l2_norm, self.linf_norm, self.linf_norm_shprg, self.b, self.current_round)

        elif args.aggregation_rule.lower() == 'zero_mask':
            global_update, self.l2_norm, self.b, benign_index = zero_mask(model_updates, self.l2_norm, self.b, self.current_round)
        
        elif args.aggregation_rule.lower() == 'jza':
            global_update, self.l2_norm, self.linf_norm, self.b, benign_index \
                = jza(model_updates, self.l2_norm, self.linf_norm, self.b, self.current_round)
